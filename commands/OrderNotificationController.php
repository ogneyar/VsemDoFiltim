        ...
        if ($emails = NoticeEmail::getEmails()) {
            foreach ($emails as $email) {
                Yii::$app->mailer->compose('admin/order', [
                        ...
                        'date' => $date,
                        'link' => 'http://будь-здоров.рус/web/admin/provider-order/date?' . 'date=' . date('Y-m-d', strtotime($date))
                    ])
                    ->setFrom(Yii::$app->params['fromEmail'])
                    ->setTo($email)
        ...
        if ($emails = NoticeEmail::getEmails()) {
            foreach ($emails as $email) {
                Yii::$app->mailer->compose('admin/order-stock', [
                        'dataProvider' => $dataProvider,
                        'date' => $date,
                        'link' => 'http://будь-здоров.рус/web/admin/order/date?' . 'date=' . date('Y-m-d', strtotime($date['end']))
                    ])
                    ->setFrom(Yii::$app->params['fromEmail'])
                    ->setTo($email)
