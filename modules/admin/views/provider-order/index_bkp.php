            ...
            <?php foreach ($models as $k => $val): ?>
                ...
                <?php if ($rowspan == 1): ?>
                    <tr>
                        ...
                        <td><a href="<?= Url::to(['/admin/provider-order/detail', 'id' => $val['id'], 'pid' => $orders[0]['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= number_format($orders[0]['quantity']); ?></a></td>
                        <td><b><?= $orders[0]['total']; ?></b></td>
                        <td><?= $val['packing']; ?></td>
                        <td><?= number_format($val['total_qnt']); ?></td>
                        <td><b><?= $val['total_price']; ?></b></td>
                        <td>
                            <?php if (ProviderNotification::find()->where(['order_date' => date('Y-m-d', strtotime($date['end'])), 'provider_id' => $val['provider_id'], 'product_id' => $val['id']])->exists()): ?>
                                Да
                            <?php else: ?>
                                Нет<br />
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($orders as $y => $order): ?>
                        <?php if ($y != 0): ?>
                            <tr>
                                <td><?= $order['p_name']; ?></td>
                                <td><a href="<?= Url::to(['/admin/provider-order/detail', 'id' => $val['id'], 'pid' => $order['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= number_format($order['quantity']); ?></a></td>
