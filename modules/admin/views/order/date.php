...
<div class="purchase-date">
            ...
            <?php foreach ($model as $k => $val): ?>
                ...
                <?php if ($rowspan == 1): ?>
                    <tr>
						...
						<?php 
							if ($val['provider_id']) $pr_id = $val['provider_id']; 
							else $pr_id = 0;
						?>
                        <td><a href="<?= Url::to(['/admin/order/detail', 'id' => $val['product_feature_id'], 'pid' => $orders[0]['p_id'], 'prid' => $pr_id, 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= ProductFeature::isWeights($val['product_feature_id']) ? $orders[0]['quantity'] : number_format($orders[0]['quantity']); ?></a></td>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($orders as $y => $order): ?>
                        <?php if ($y != 0): ?>
                            <tr>
                                <td><?= $order['p_name']; ?></td>
                                <td><a href="<?= Url::to(['/admin/order/detail', 'id' => $val['product_feature_id'], 'pid' => $order['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= number_format($order['quantity']); ?></a></td>
    ...
    <?= Html::a('Детализация', 'javascript:void(0)', ['class' => 'btn btn-success closed', 'onclick' => 'detalizationStock();', 'id' => 'purchase-details-btn']) ?>
    <?= Html::a('Удалить', Url::to([$delete_action, 'date' => date('Y-m-d', strtotime($date['end']))]), [
                'class' => 'btn btn-danger pull-right',
                'data-pjax' => 0,
                'data-method' => "post",
                'data-confirm' => "Вы уверены что хотите удалить заказ?"
            ]) ?>
    <input type="hidden" id="details-date-start" value="<?= $date['start']; ?>">
    <input type="hidden" id="details-date-end" value="<?= $date['end']; ?>">
