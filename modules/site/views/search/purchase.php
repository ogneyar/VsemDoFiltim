...
<div class="order-index">
    <?php foreach ($dataProvider->getModels() as $model): ?>
        ...
        <p style="text-decoration: underline;">Заказчик: <strong><?= empty($model->role) ? "ГОСТЬ" : "УЧАСТНИК" ?></strong> <a href="<?= isset($model->user->member) ? Url::to(['/profile/partner/member/update', 'id' => $model->user->member->id]) : (isset($model->user->partner) ? Url::to(['/admin/partner/view', 'id' => $model->user->partner->id]) : "") ?>"><?= Html::encode($model->fullName) ?> № Регистрации: <?= $model->user->number ?></a></p>
