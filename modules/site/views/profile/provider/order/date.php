...
<div class="purchase-date">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($model as $k => $val): ?>
                <?php $orders = PurchaseOrder::getOrderByProductByPartner($val['product_feature_id'], $date, $partner_id); ?>
                ...
                <?php if ($rowspan == 1): ?>
                    <tr>
                        ...
                        <td><a href="<?= Url::to(['detail', 'id' => $val['product_feature_id'], 'pid' => $orders[0]['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date))]); ?>" style="text-decoration: underline;"><?= number_format($orders[0]['quantity']); ?></a></td>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($orders as $y => $order): ?>
                        <?php if ($y != 0): ?>
                            <tr>
                                <td><?= $order['p_name']; ?></td>
                                <td><a href="<?= Url::to(['detail', 'id' => $val['product_feature_id'], 'pid' => $order['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date))]); ?>" style="text-decoration: underline;"><?= number_format($order['quantity']); ?></a></td>
    ...
    <?= Html::a('Назад', Url::to(['index']), ['class' => 'btn btn-info']) ?>
